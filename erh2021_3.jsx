...
Application.prototype.erhIdleTasks = function erhIdleTasks(cmd){
      ...
      switch (cmd) {
            ...
            case 'switchDoc':
                  //alert('erhIdleTasks: switchDoc: ' + erhIdleTasks.task.isValid);
                  ...
                  //alert('erhIdleTasks: new task started');
                  break;
            default:
                  alert('Cmd: ' + cmd);
                  break;
...
Document.prototype.erhIdleTask = function erhIdleTask(event){ //this is CALLBACK
      ...
      erhIdleTask.renew(erhIdleTask._this.pages.length, function(prev, curr) {
                  //alert('renew: prev = ' + prev + ', curr = ' + curr);
...
Document.prototype.docDeleteHelperLayers = function(){
      ...
      var   m_layer = this.layers.itemByName('erh_m_layer'),
            ...
            this.masterSpreads.everyItem().pageItems.everyItem().getElements()
            .map(function(pi){
                  pi.visible = true;
                  return pi;
            })
            .filter(function(pi){
                  return pi.itemLayer.id == m_layer.id;
            })
            .forEach(function(pi){
                  pi.locked = false;
                  ...
                  try {
                        ...
                        if (piOrig && piOrig instanceof PageItem) {
                              piOrig.locked = false;
                              piOrig.geometricBounds = pi.geometricBounds; //If user has shifted the copy, we should shift the origin
                              piOrig.visible = true;
                              pi.remove();
                        }
                  }
...
Document.prototype.makeDuplicates2WLayer = function(){
      ...
      try {
            ...
            .forEach(function(pg){
                  var m = pg.appliedMaster;
                  pg.appliedMaster = NothingEnum.NOTHING;
                  pg.appliedMaster = m;
            });
      }
      catch(e){ alert(e); }
